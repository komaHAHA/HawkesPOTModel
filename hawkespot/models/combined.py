...
class HawkesPOTModel(nn.Module):
    def __init__(self, hawkes_process, 
                pot_model,
                start: float, 
                end: float, 
                vec_times: torch.Tensor, 
                marks: torch.Tensor):
        super().__init__()
    ...
    def log_likelihood(self) -> torch.Tensor:
        intensities = self.hawkes.intensity(self.data_matr_filtered, self.marks, self.mask)
        hawkes_ll = torch.sum(torch.log(intensities)) - self.hawkes.integral(self.start, self.end, self.vec_times, self.marks)

        pot_ll = torch.sum(self.pot.log_density(self.data_matr_filtered, self.marks, self.mask))
        
        return hawkes_ll + pot_ll
